...
只有 CLI 版本确实变化时才发送可选通知（默认启用）；仅凭据恢复或改用 Desktop 会话不会打扰用户。没有任何可用会话时，凭据缺失、不可续刷和服务端拒绝统一显示**认证失败**，并提示用户重新登录 Claude。Claude Desktop 用户可先发送一条消息，让正常会话流程尝试刷新凭据；若监控仍未恢复，再退出并重新登录。Claude Code CLI 用户可在终端运行 `claude auth login`。登录后，凭据监视会自动恢复用量监控。
