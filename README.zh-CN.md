...
从旧版本升级时不会再次弹出授权框，已经明确允许的服务商保持允许。旧文件里只有 `allow=false`、无法判断你是否主动撤销的服务商会标为**待复核**，在你于服务商访问权限中选择**允许访问**或**保持关闭**之前，所有静默读取路径都不会碰它。**重新探测服务商**会对每个待复核项再问一次，并说明会读取该服务商的本地凭据用于持续监控。升级后新增、且并未进入待复核或已撤销的服务商仍可自动发现。已允许的新装服务商可用**服务商访问权限 → 重新探测服务商**立即纳入。此后更筹会在每次启动后不久检查一次、之后定期检查，发现新的已登录服务商时只弹一次通知，不会自行改变显示内容。启动时的这次检查是为了让升级后新增的服务商能及时告知你，而不必等满一个检查周期。
