...
- **任务栏小组件**嵌入任务栏本体。每家服务商对应一个内容自适应的单行徽章，显示 logo、配额窗口标签、用量和重置倒计时。悬停徽章可查看该服务商报告的所有配额窗口及重置时间；拖动左侧分隔线可调整位置，拖到另一条任务栏即可切换显示器。Explorer 暂时不可用时，小组件会保持隐藏，并在任务栏恢复后重新嵌入。不支持竖排任务栏：徽章按横向任务栏排布，比停靠在左侧或右侧的任务栏更宽，这种布局下请改用桌面浮窗或托盘图标。
...
- **托盘图标**会为每个已启用的服务商显示一枚实时图标。数字和自适应量条取自接口实际返回的配额窗口；暂无数据时显示服务商首字母。关闭**服务商托盘图标**后只保留一个中性软件图标。此外，当没有任何服务商获得凭据访问权限时也会显示软件图标——此时摆出任何一家的标志都不诚实；你的偏好不会被改写，任何一家重新获授权后服务商图标就会回来。
...
发现新服务商、配额重置、发现新版本和 Claude Code 更新都属于例行通知：使用更筹应用图标并保持静音。只有当前凭据确实需要用户处理时，才使用 Windows 警告字形和通知声音。

更筹每天检查一次自身更新，同一个新版本只提示一次——每天重复提醒是打扰。点击该通知会询问是否现在更新。错过也不要紧：详情弹窗页脚会一直写着 `v2.5.3 → 2.5.4`，点这行字会问同样的问题。**设置**里的版本项保留同一个入口，也是键盘用户的通路。
...
首次启动时，更筹会请求一次授权，说明访问仅用于查询用量、不会消耗模型额度、也不会保存登录信息；默认选项为**不允许**，未获授权前不会读取任何凭据。授权后更筹会检查本机有哪些服务商已登录，并显示检查到的那些；如果一个也没探测到，会保留一个仅在本地轮询的 Codex 占位项，以便识别首次登录。授权是一次性的、覆盖全部服务商，但撤销仍然是按服务商的：可随时在右键菜单的**服务商访问权限**中单独关闭某一个。关闭之后该服务商的凭据就完全不再被读取：启动后的那次检查、定期检查和**重新探测服务商**都不会碰它，直到你重新打开。**服务商**菜单里的显示/隐藏只改变是否出现在界面上，不会授予凭据访问。授权后，更筹仍会按需重新读取原文件或 Windows 凭据管理器条目，因此服务商自动刷新令牌的机制可以继续工作，而登录信息不会保存到更筹中。

从旧版本升级时不会再次弹出授权框，已经明确允许的服务商保持允许。旧文件里只有 `allow=false`、无法判断你是否主动撤销的服务商会标为**待复核**，在你于服务商访问权限中选择**允许访问**或**保持关闭**之前，所有静默读取路径都不会碰它。复核框还有第三项**稍后再说**，也是默认项：它什么都不改、保持待复核，所以关掉对话框或误按回车都不会替你做决定。**重新探测服务商**会对每个待复核项再问一次，并说明会读取该服务商的本地凭据用于持续监控。升级后新增、且并未进入待复核或已撤销的服务商仍可自动发现。已允许的新装服务商可用**服务商访问权限 → 重新探测服务商**立即纳入。此后更筹会在每次启动后不久检查一次、之后定期检查，发现新的已登录服务商时只弹一次通知，不会自行改变显示内容。启动时的这次检查是为了让升级后新增的服务商能及时告知你，而不必等满一个检查周期。

`gengchou.exe --claude-auth-diagnostics` 是用户主动触发的例外：只为打印脱敏诊断报告而读取本机 Claude 凭据。

如果一个服务商本机没有任何登录凭据，详情弹窗会显示**未检测到**并提示登录后自动识别，不会弹出通知——从未登录过的服务商没有需要「重新登录」的东西。**认证失败**表示凭据确实存在却不能用：Windows 侧的文件或凭据管理器条目无法读取、格式无效，或者令牌已经过期、被服务端拒绝；这些情况对用户都归为同一个简单恢复动作：重新登录。WSL 探测程序本身无法启动或没有按时完成时，一律按暂时刷新失败处理，不会误报认证问题。在 WSL 内部，Claude 的探测仍能区分「没有凭据」和「有但读不了」；Codex、Antigravity 与 Grok 的探测无法区分「这个发行版里本来就没有凭据」和「这个发行版没能按时应答」，只会继续尝试下一个来源，因此凭据只存在于当前无法访问的发行版中时，这三家显示的是**未检测到**，而不是误报一条警告式的认证通知。
...
| 空文件，它的独占句柄就是单实例守卫；不会向其写入任何内容 | `%APPDATA%\Gengchou\instance.lock` |
