...
如果一个服务商本机没有任何登录凭据，详情弹窗会显示**未检测到**并提示登录后自动识别，不会弹出通知——从未登录过的服务商没有需要「重新登录」的东西。**认证失败**表示凭据确实存在却不能用：Windows 侧的文件或凭据管理器条目无法读取、格式无效，或者令牌已经过期、被服务端拒绝；这些情况对用户都归为同一个简单恢复动作：重新登录。WSL 探测不做这种判定——它无法区分「这个发行版里本来就没有凭据」和「这个发行版没能按时应答」，只会继续尝试下一个来源；因此凭据只存在于当前无法访问的发行版中时，显示的是**未检测到**，而不是误报一条警告式的认证通知。
