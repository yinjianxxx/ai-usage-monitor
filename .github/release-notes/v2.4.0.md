...
> **Repository correction note (2026-08-09):** the public GitHub Release body
> is immutable and remains the exact published text. This tracked copy includes
> a post-publication clarification of the no-provider Codex placeholder. It is
> retained as the corrected repository record, not evidence that the public
> body changed.
...
- Replace the per-provider first-run prompts with a single permission prompt
  ...
  on this machine and show those. If none are detected, keep a locally polled
  Codex placeholder visible so the first sign-in can be recognized without
  changing the user's provider selection. Nothing is read before permission is
